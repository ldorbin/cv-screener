...
import AdminOrgClient from "./client";
...
export default async function AdminOrgDetailPage(props: { params: Promise<{ id: string }> }) {
  ...
  const userMap = Object.fromEntries(users.map((u) => [u.id, u.email ?? u.id]));
  ...
  return (
    <div>
      ...
      <h1 className="text-3xl font-bold text-gray-900 mb-8">{org.name}</h1>
      ...
      <div className="grid grid-cols-2 gap-8 mb-8">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <AdminOrgClient orgId={id} currentPlan={org.plan} currentStatus={org.plan_status} />
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-200">
            {(members ?? []).map((m) => (
              <tr key={m.id}>
                <td className="px-6 py-3 text-sm text-gray-900">{userMap[m.user_id] ?? m.user_id}</td>
